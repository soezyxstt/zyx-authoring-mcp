...
4. Panggil `source.list_reference_files` untuk melihat PDF kategori `soal` dan `pembahasan_soal`. Gunakan `source.read_file` dengan file key yang dikembalikan untuk membaca halaman, teks, dan gambar sumber.
5. Panggil `knowledge.search_course_ideas` untuk mencari Idea published di seluruh bab course. Panggil `knowledge.get_idea` untuk membaca statement, provenance, dan label chapter sebelum menautkan Idea.
